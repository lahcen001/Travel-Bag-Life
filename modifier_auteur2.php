...
$fileError = $_FILES['avatar']['error'];
  ...
  $email =( $_POST['email']);
  $sql = 'UPDATE auteur SET name=:name, avatar=:avatar , id_auteur=:id_auteur , email=:email WHERE id_auteur=:id_auteur';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':name' => $name, ':avatar' => $fileName1, ':id_auteur' => $id_auteur, ':email' => $email])) {
...
<div class="card mt-4 mb-4  shadow rounded justify-content-center">
  <div class="card-body">
      ...
      <form method="post"  enctype="multipart/form-data">
        ...
        <img class="card-img-top pt-3 w-25 shadow  img-fluid img-thumbnail " src="trait_auteurs\auteurs_image\<?= $table->avatar; ?>" alt="Card image cap">
